...
public class UserOrdersController {
    ...
    @ResponseBody
    public ResponseEntity<?> confirmReceived(@PathVariable Long id, @AuthenticationPrincipal User user) {
        if (user == null) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "Vui lòng đăng nhập."));
        }
        try {
            orderService.confirmDelivery(id, user.getUsername());
            return ResponseEntity.ok(Map.of("message", "Xác nhận đã nhận hàng thành công cho đơn hàng #" + id + "."));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> requestReturn(
        ...
        try {
            ...
            if (evidenceFile != null && !evidenceFile.isEmpty()) {
                try {
                    evidenceFilename = fileStorageService.save(evidenceFile);
                } catch (Exception e) {
                    log.error("Failed to save return evidence file: {}", e.getMessage());
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Không thể tải lên tệp minh chứng."));
                }
            }
            orderService.returnOrder(id, user.getUsername(), reason, evidenceFilename);
            return ResponseEntity.ok(Map.of("message", "Yêu cầu trả hàng/hoàn tiền thành công cho đơn hàng #" + id + "."));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", e.getMessage()));
        }
    }
