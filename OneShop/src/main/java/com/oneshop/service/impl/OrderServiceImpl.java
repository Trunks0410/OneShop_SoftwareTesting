...
public class OrderServiceImpl implements OrderService {
    ...
    public List<Order> findOrdersByCurrentUser(String username) {
        ...
        try {
            ...
            // Auto-confirm orders delivered by shipper > 5 days ago
            autoConfirmOverdueOrdersForUser(currentUser.getId());
    ...
    public void updateShipperOrderStatus(Long orderId, Long shipperId, OrderStatus newStatus) {
        ...
        if (newStatus == OrderStatus.DELIVERED) {
            // Shipper confirms delivery: set shipperDelivered flag and timestamp, but keep orderStatus as DELIVERING
            order.setShipperDelivered(true);
            order.setShipperDeliveredAt(LocalDateTime.now());
            orderRepository.save(order);
            logger.info("Order {} marked as shipperDelivered by shipper {}", orderId, shipperId);
            return;
        }
    ...
    private void autoConfirmOverdueOrdersForUser(Long userId) {
        try {
            LocalDateTime threshold = LocalDateTime.now().minusDays(5);
            List<Order> overdueOrders = orderRepository.findByUserIdAndOrderStatusAndShipperDeliveredAndShipperDeliveredAtBefore(
                userId, OrderStatus.DELIVERING, true, threshold
            );
            for (Order order : overdueOrders) {
                order.setOrderStatus(OrderStatus.DELIVERED);
                orderRepository.save(order);
                logger.info("Order #{} automatically confirmed after 5 days from shipper delivery.", order.getId());
                try {
                    if (order.getUser() != null && order.getUser().getEmail() != null) {
                        emailService.sendDeliveryConfirmation(order.getUser().getEmail(), order.getId());
                    }
                } catch (Exception e) {
                    logger.error("Error sending auto delivery confirmation email for order {}: {}", order.getId(), e.getMessage());
                }
            }
        } catch (Exception e) {
            logger.error("Error running auto-confirm overdue orders for user {}: {}", userId, e.getMessage(), e);
        }
    }
    ...
    public void confirmDelivery(Long orderId, String username) {
        logger.info("User {} confirming delivery for order {}", username, orderId);
        Order order = findOrderByIdAndUser(orderId, username);

        if (order.getOrderStatus() != OrderStatus.DELIVERING) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, 
                "Đơn hàng phải ở trạng thái đang giao mới có thể xác nhận nhận hàng.");
        }
    ...
    @Override
    public void returnOrder(Long orderId, String username, String reason, String evidenceFilename) {
        logger.info("User {} requesting return/refund for order {} with reason: {} and evidence: {}", username, orderId, reason, evidenceFilename);
        Order order = findOrderByIdAndUser(orderId, username);

        OrderStatus currentStatus = order.getOrderStatus();
        if (currentStatus != OrderStatus.DELIVERING && currentStatus != OrderStatus.DELIVERED) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, 
                "Đơn hàng phải ở trạng thái đang giao hoặc đã giao mới có thể trả hàng/hoàn tiền.");
        }

        // Kiểm tra thời hạn 7 ngày kể từ khi shipper giao hàng
        if (order.getShipperDeliveredAt() != null) {
            if (order.getShipperDeliveredAt().plusDays(7).isBefore(LocalDateTime.now())) {
                throw new ResponseStatusException(HttpStatus.BAD_REQUEST, 
                    "Đã quá hạn 7 ngày kể từ lúc nhận hàng để yêu cầu Trả hàng/Hoàn tiền.");
            }
        }

        order.setOrderStatus(OrderStatus.RETURNED);
        order.setReturnReason(reason);
        order.setReturnEvidence(evidenceFilename);
        ...
        orderRepository.save(order);
        logger.info("Order {} successfully returned by user {}", orderId, username);
    }
}